...
public:
    BatteryBalance();
    void begin(Stream *serial,SerialManager serialManager);
    void run();
...
private:
    ...
    uint8_t error;                                     //錯誤碼
    int balance = Take_break_balance;
    ...
    void applySPISpeed();
    void restoreSPISpeed();

    float num_cell = 0.0, high_vlotage = 0.0, low_vlotage = 5.0;
    float num_high_cell = 0.0, avage_high_voltage = 0.0, all_high_voltage = 0.0;
    float num_low_cell = 0.0, avage_low_voltage = 0.0, all_low_voltage = 0.0;
};
